...
if ( mrQ.B1fit_done==0)
    ...
    if isfield(mrQ,'B1_interp_all') 
        if mrQ.B1_interp_all==1
            fprintf('\n B1 interpolation on the entire brain              \n');
            EPI_BMmask=readFileNifti(mrQ.SEIR_epi_Maskfile); EPI_BMmask=EPI_BMmask.data;  % This is the brain mask in the EPI space
            maskEPI=readFileNifti(mrQ.maskepi_File); % the B1 interpolation mask
            maskEPI.data(EPI_BMmask>0)=1;  % add the entire brain to the interpolation mask
            dtiWriteNiftiWrapper(maskEPI.data,maskEPI.qto_xyz,mrQ.maskepi_File); % save new interpolation mask
        end
    end
...
if  mrQ.B1correct_done==0;
    ...
    mrQ=mrQ_T1M0_Fit_WL_uncorrected(mrQ,mrQ.B1.unCorrected);
